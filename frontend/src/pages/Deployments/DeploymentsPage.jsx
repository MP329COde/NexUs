  ...
  return (
    <>
      <PageHeader
        ...
        actions={<button className="btn" onClick={() => setFormOpen(true)}>Lier une application</button>}
      />

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12,1fr)', gap: 16 }}>
        <Panel title="Applications suivies" span={12}>
          <DataTable
            columns={['Nom', 'Projet GitLab', 'Application Argo CD', 'Déploiement K8s', '']}
            rows={links.data?.items}
            emptyTitle="Aucune application liée"
            emptyHint="Reliez un projet GitLab, une application Argo CD et un déploiement Kubernetes pour suivre le pipeline complet."
            renderRow={(l) => (
              <tr key={l.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(l.id)}>
                <td style={{ fontWeight: 500 }}>{l.name}</td>
                <td className="mono muted">{l.gitlabProjectId || '—'}</td>
