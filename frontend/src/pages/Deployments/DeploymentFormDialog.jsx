...
import './DeploymentFormDialog.css';
      ...
      <form id="deployment-form" onSubmit={onSubmit}>
        ...
        {form.gitProvider === 'gitlab' ? (
          ...
          <div className="dfd-row">
            <div className="dfd-row-field"><Field label="Propriétaire GitHub"><input className="input" placeholder="org-ou-utilisateur" value={form.githubOwner} onChange={(e) => set('githubOwner', e.target.value)} /></Field></div>
            <div className="dfd-row-field"><Field label="Dépôt GitHub"><input className="input" placeholder="mon-app" value={form.githubRepo} onChange={(e) => set('githubRepo', e.target.value)} /></Field></div>
          </div>
        )}
        ...
        <div className="dfd-row">
          <div className="dfd-row-field"><Field label="Namespace K8s"><input className="input" value={form.k8sNamespace} onChange={(e) => set('k8sNamespace', e.target.value)} /></Field></div>
          <div className="dfd-row-field"><Field label="Deployment K8s"><input className="input" value={form.k8sDeployment} onChange={(e) => set('k8sDeployment', e.target.value)} /></Field></div>
        </div>

        {error && <div className="dfd-error">{error}</div>}
      </form>
    </Modal>
  );
}
...
function Field({ label, children }) {
  return (
    <div className="dfd-field">
      <label className="dfd-field-label">{label}</label>
      {children}
    </div>
  );
}
