...
export default function AccountPage() {
  ...
  async function saveProfile(e) {
    ...
    try {
      await updateProfile({ name, avatarEmoji: emoji, avatarColor: color });
        ...
        <Panel title="Profil" span={6}>
          <form onSubmit={saveProfile} style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
              {EMOJIS.map((e) => (
                <span key={e} onClick={() => setEmoji(e === emoji ? '' : e)} style={{ width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, cursor: 'pointer', fontSize: 15, background: e === emoji ? 'var(--primary-soft)' : 'var(--border-soft)', border: e === emoji ? '1px solid var(--primary)' : '1px solid transparent' }}>{e}</span>
              ))}
            </div>
