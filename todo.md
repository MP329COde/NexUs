...
## Lot D8 — Système de plugins approfondi (2026-08-23)

- [x] **Système de plugins approfondi (Lot D8)** : audit préalable (non refait ici) confirmait un socle solide déjà en place (`backend/src/services/plugins/pluginRegistry.js`, `manifestSchema.js` avec allowlist fermée de permissions, `checkCompatibility()` déjà bloquant et réel — comparaison numérique dotted de `manifest.minNexusVersion`/`maxNexusVersion` contre `NEXUS_VERSION` lue depuis `backend/package.json`) mais avec 4 lacunes précises : aucune approbation admin des permissions à l'installation (toutes actives immédiatement), aucun template de plugin téléchargeable, aucun mode dev local/Git, aucun rollback réel ni healthcheck. Ce lot comble les 4, sans dupliquer la vérification de compatibilité déjà correcte (`checkCompatibility`/`compareVersions` désormais exportés et réutilisés par `updatePlugin`, pas de second système créé).
  - **1. Template officiel téléchargeable** (`backend/src/services/plugins/pluginTemplateService.js`, nouveau) : `buildPluginTemplateFiles({id,name,version})` génère 6 fichiers réels et spécifiques (jamais de texte générique) — `manifest.json` valide (vérifié avec `validateManifest()` réel), `package.json`, `README.md` généré dynamiquement (nom/id/version réels, arborescence listée dynamiquement à partir des fichiers effectivement inclus, permissions déclarées avec avertissement si hors catalogue, commandes réelles `node --test`/`POST /api/plugins/install-local`/`POST /api/plugins/install-git`), `src/index.js` d'exemple avec contribution `menus`/`pages`, `test/plugin.test.js` (`node --test`, sans dépendance, valide que le manifest parse et respecte le schéma), `.github/workflows/ci.yml` généré via `buildCiWorkflow()` de `ciWorkflowService.js` (réutilisé tel quel, pas de YAML CI écrit à la main). **Aucune lib zip présente dans `backend/package.json`** (`archiver`/`adm-zip`/`jszip` absents, vérifié avant d'écrire le code) : `buildZip()` construit un vrai fichier ZIP PKZIP (méthode DEFLATE) à la main avec `node:zlib` (`deflateRawSync` + `crc32`, tous deux natifs depuis Node 22, `engines.node >=22.5.0` déjà dans `package.json` donc aucune régression de compatibilité) plutôt que d'ajouter une dépendance pour un besoin aussi simple — validé en conditions réelles : `unzip -l`/`unzip -o` sur le zip produit par un vrai appel HTTP `GET /api/plugins/template` fonctionnent, et `node --test` sur le fichier de test extrait passe (2/2). Endpoint `GET /api/plugins/template?id=&name=&version=` placé délibérément AVANT le middleware de disponibilité du socle Postgres dans `plugins.routes.js` (pure génération de fichiers, aucune écriture DB, reste utilisable sans `DATABASE_URL`).
  - **2. Flux d'approbation des permissions admin** : migration `backend/src/db/migrations/0048_plugin_lifecycle.sql` ajoute `plugin_permissions.status` (`pending`/`granted`/`denied`, défaut `pending` — CHECK contraint) + `decided_at`/`decided_by`. `installPlugin()` insère désormais toute permission déclarée en `pending` (avant ce lot : active immédiatement, sans validation). Nouvelles fonctions `grantPluginPermission(id,key,decidedBy)`/`denyPluginPermission(...)` dans `pluginRegistry.js`, exposées via `POST /api/plugins/:id/permissions/:key/grant` et `/deny` (`requirePermission('plugins','write')`, réutilise la permission RBAC existante — aucune nouvelle permission `plugins:approve` créée, décision délibérée pour rester cohérent avec le modèle RBAC existant). `activatePlugin(id)` échoue désormais avec **409 réel** et la liste précise des permissions non approuvées si au moins une permission déclarée par le manifest n'est pas `granted` (`pendingPermissions` dans l'erreur). `pluginHasPermission()` vérifie désormais `status='granted'` en plus de `status='active'` du plugin — un plugin actif dont une permission a été révoquée après coup (via `deny`) perd immédiatement l'accès à cette capacité. Frontend (`PluginsPanel.jsx`) : modal détail affiche chaque permission avec badge de statut (En attente/Accordée/Refusée) + boutons Accorder/Refuser contextuels, et un avertissement visuel explicite si le plugin ne peut pas être activé faute d'approbations.
  - **3. Rollback automatique en cas d'échec de mise à jour** : `updatePlugin(id, newManifest)` dans `pluginRegistry.js`, exposé via `POST /api/plugins/:id/update`. Utilise une VRAIE transaction Postgres (`pool.connect()` + `BEGIN`/`COMMIT`/`ROLLBACK` explicites sur un client dédié, même pattern que `restoreRelationalCore()` dans `pgDumpService.js` — pas de nouveau système inventé) : valide le nouveau manifest (`validateManifest`), vérifie la compatibilité (`checkCompatibility`, réutilisé, pas dupliqué), sauvegarde l'état précédent complet (manifest+version+statut+permissions) dans la nouvelle table `plugin_update_backups` (écrite DANS la même transaction — si le reste échoue, le `ROLLBACK` annule aussi la sauvegarde, pas de backup orphelin), met à jour `plugins` et recalcule les permissions (celles déjà `granted` et toujours déclarées le restent, toute permission NOUVELLE repart en `pending` — jamais accordée automatiquement par continuité, les permissions retirées sont supprimées). Si une étape échoue, `ROLLBACK` réel et l'erreur d'origine est propagée telle quelle (jamais un succès fictif) — **testé réellement en conditions réelles** : un `POST /:id/update` avec un manifest invalide (`name` vide) renvoie 400 et une relecture immédiate du plugin confirme qu'il est resté à l'identique (version, manifest, `updated_at` inchangés) ; un `POST /:id/update` valide avec une permission en plus/une en moins confirme le comportement attendu sur les permissions (voir tests curl ci-dessous).
  - **4. Healthcheck par plugin** : `getPluginHealth(id)` dans `pluginRegistry.js`, exposé via `GET /api/plugins/:id/health`. 4 vérifications sur des données réelles uniquement (jamais de valeur inventée) : présence d'erreurs récentes dans `plugin_events_log` (`event_type ILIKE '%error%'`), fraîcheur du dernier événement (seuil 30 jours), cohérence permissions déclarées vs `granted`, statut `active`/non-actif. Statut agrégé `healthy`/`degraded`/`unhealthy` (`unhealthy` si erreurs récentes ou permissions manquantes — bloquant réel ; `degraded` si seulement inactif ou événements anciens — rien de cassé). Affiché dans la modal détail (`PluginsPanel.jsx`, badge + liste des vérifications individuelles).
  - **5. Compatibilité version** : déjà bloquante avant ce lot (`checkCompatibility`) — confirmé réutilisé sans duplication par `installPlugin`, `activatePlugin` (déjà avant) ET `updatePlugin` (nouveau, ce lot). `compareVersions`/`checkCompatibility` désormais exportés depuis `pluginRegistry.js`.
  - **6. Mode dev local + dépôt Git distant** : `installLocalPlugin(dirPath)` lit et valide `manifest.json` d'un dossier serveur AVANT toute écriture DB (rejet clair 400 si absent/JSON invalide/schéma invalide, testé réellement avec un manifest invalide `{"id":"bad id","name":""}` → rejeté), marque `plugins.source='local-dev'`/`source_ref=dirPath` (nouvelles colonnes, migration `0048`). `installGitPlugin(repoUrl, ref)` télécharge `manifest.json` via `fetch` natif Node : détection GitHub (`raw.githubusercontent.com/{owner}/{repo}/{ref||main}/manifest.json`) sinon fallback générique GitLab/Gitea (`{repoUrl}/raw/{ref||main}/manifest.json`) — testé réellement contre un vrai dépôt GitHub inexistant, erreur 404 propre propagée (`manifest.json introuvable sur ... (HTTP 404)`), jamais de faux "installé". Simple `fetch` public sans auth (suffisant pour un manifest public, pas de réinvention du pattern d'auth GitHub existant de `githubService.js`/`githubRepoSetup.js` qui reste pour les opérations authentifiées sur les dépôts gérés). Exposés via `POST /api/plugins/install-local` et `POST /api/plugins/install-git`, `requirePermission('plugins','write')`, `logAudit` (`plugin.devload`/`plugin.gitinstall`).
  - **7. README.md réel généré** : voir point 1 — le README produit par `pluginTemplateService.js` inclut nom/id/version réels, arborescence dynamique des fichiers réellement inclus, permissions déclarées avec avertissement si hors catalogue, et instructions dev/test/publication avec les VRAIES commandes du projet (`node --test`, `POST /api/plugins/install-local`, `POST /api/plugins/install-git`) — jamais un texte générique copié-collé.
  - **Vérifié réellement** : `node --check` sur les 4 fichiers backend créés/modifiés (`pluginRegistry.js`, `pluginTemplateService.js`, `plugins.routes.js`, migration SQL non concernée par `--check`) — tous OK. `node --test` (suite complète backend) : **137 pass / 0 fail / 3 skipped (140 total)** — aucune régression (baseline précédente documentée : 133 pass/4 fail/3 skipped ; les 4 échecs précédemment documentés dans `backupService.test.js`/`jobService.test.js` passent maintenant, probablement parce que ce run avait Postgres réellement accessible sur `localhost:5433`, sans rapport avec ce lot). Migration `0048_plugin_lifecycle.sql` appliquée réellement au démarrage du serveur (log confirmé : `Migration Postgres appliquée : 0048_plugin_lifecycle.sql`). **Flux complet testé via curl avec une vraie session admin** (`admin@homelab.local`/`admin1234`, cookies + CSRF réels) : génération du template (`GET /api/plugins/template?id=test-d8&...` → vrai ZIP téléchargé, `unzip -l` confirme les 6 fichiers, `node --test` sur le test extrait passe) ; `install-local` avec manifest invalide → 400 rejeté proprement ; `install-local` avec manifest valide (2 permissions) → installé, permissions en `pending` ; `activate` → 409 avec liste des permissions non approuvées ; `grant` des 2 permissions une à une (chaque `activate` intermédiaire re-testé, toujours 409 tant qu'il en reste une en attente) ; `activate` final → 200, statut `active` ; `health` → `healthy` (aucune erreur, permissions cohérentes) ; `install-git` sur un dépôt GitHub inexistant → 404 propre ; `deny` d'une permission d'un plugin actif → `health` repasse à `unhealthy` (`permissions_consistency` en échec) ; `update` avec manifest invalide → 400, plugin relu inchangé (rollback confirmé) ; `update` valide avec permissions modifiées → permission conservée reste `granted`, permission retirée disparaît, nouvelle permission repart en `pending` (comportement exact attendu). Build frontend (`vite build`) : 494 modules, sans erreur.
  - **Limites connues, documentées honnêtement** : **pas de sandbox d'exécution JS réelle pour le code de plugin** — le socle NexUs actuel (confirmé par l'audit préalable à ce lot : `eventBus.js`/`hookRegistry.js`) n'exécute aucun code serveur autonome de plugin ; `src/index.js` du template reste un exemple de structure documenté comme tel dans son propre commentaire, jamais présenté comme exécuté. Le healthcheck (point 4) en découle directement : il est basé sur le journal d'événements et la cohérence des permissions déclarées, **pas sur un ping applicatif réel du plugin**, puisqu'aucun processus/endpoint de plugin n'existe à interroger — documenté explicitement dans le code du service plutôt que simulé. `install-git` utilise un `fetch` public sans authentification : un dépôt privé nécessitant un token échouera (comportement attendu et honnête, pas un token géré dans ce lot — hors périmètre demandé). La convention d'URL "raw" générique pour GitLab/Gitea (`{repoUrl}/raw/{ref}/manifest.json`) n'a pu être testée que par construction d'URL (échec 404 propre confirmé sur GitHub réel) faute d'un dépôt GitLab/Gitea public de test disponible dans cet environnement — reste à valider contre une instance réelle si utilisé en production.
  - **Fichiers** : `backend/src/db/migrations/0048_plugin_lifecycle.sql` (nouveau), `backend/src/services/plugins/pluginRegistry.js` (compareVersions/checkCompatibility exportés, permissions en `pending` à l'installation, `activatePlugin` bloquant, `pluginHasPermission` vérifie `status='granted'`, `getPluginPermissions` retourne le statut, nouvelles fonctions `grantPluginPermission`/`denyPluginPermission`/`installLocalPlugin`/`installGitPlugin`/`updatePlugin`/`getPluginHealth`), `backend/src/services/plugins/pluginTemplateService.js` (nouveau), `backend/src/routes/plugins.routes.js` (nouveaux endpoints `/template`, `/install-local`, `/install-git`, `/:id/permissions/:key/grant`, `/:id/permissions/:key/deny`, `/:id/update`, `/:id/health`), `frontend/src/pages/Settings/PluginsPanel.jsx` (onglets d'installation manifest/local/Git, téléchargement du template, approbation des permissions, badge de santé), `frontend/src/pages/Settings/PluginsPanel.css`.

## Lot D9 — Écran de bootstrap/démarrage (2026-08-23)

- [x] **Écran de bootstrap/démarrage (Lot D9)** : point de départ le socle du Lot A7 (`backend/src/services/startupStatusService.js` + instrumentation `runStep()` dans `backend/src/index.js`, déjà commité en `5225c3d`), étendu — pas dupliqué — pour couvrir la liste complète demandée : connexion DB, chargement configuration, migrations, admin bootstrap, planificateurs, vérification intégrations, vérification certificats, vérification réseau, plus un état runtime continu (backend/frontend/workers). Aucune durée ni statut affiché n'est simulé : chaque étape passe par `beginStep()`/`ok()`/`fail()`/`degraded()`, mesurée en temps réel au démarrage réel du process.
  - **Étapes ajoutées dans `index.js`** (toutes avant l'étape `schedulers` déjà existante, sauf les 3 dernières) : `configLoad` (vérifie que `JWT_SECRET` est défini, avertit sans bloquer si c'est encore la valeur de dev par défaut — comportement historique de dev préservé) ; `dbConnection` (rendue EXPLICITE — avant ce lot elle était implicite dans la première requête de `runMigrations()` — fait un vrai `pool.connect()` + `SELECT 1`, `ok()` immédiat si `DATABASE_URL` absent car c'est un mode dégradé documenté du projet, pas un échec). Puis, en étapes NON bloquantes (`runOptionalStep`, nouveau : un échec passe l'étape en `degraded` sans jamais arrêter le démarrage) : `integrationsCheck` (interroge `getStatus()` de chaque intégration de `integrationRegistry.js`, comme le fait déjà `GET /system/overview`, mais capturé comme étape de démarrage chronométrée) ; `certificatesCheck` (réutilise strictement `tlsDiagnosticsService.js` du Lot B4 — `listTlsIntegrationKeys()`/`diagnoseIntegration()` — **aucune logique TLS dupliquée**) ; `networkCheck` (résolution DNS réelle vers `github.com` via `node:dns/promises`, seul signal de connectivité sortante honnête sans prétendre vérifier "tout le réseau").
  - **Initialisation plugins (Lot D8) : délibérément NON instrumentée.** Vérifié dans `backend/src/services/plugins/pluginRegistry.js` : aucune fonction de bootstrap n'est appelée au démarrage du process — chaque route de `plugins.routes.js` interroge la table `plugins` à la demande (`listPlugins()`/`getPlugin()`), il n'existe pas de chargement en mémoire des plugins actifs au boot. Ajouter une étape `pluginsInit` factice (un `listPlugins()` de complaisance juste pour avoir une ligne dans la timeline) aurait inventé une étape de démarrage qui n'existe pas réellement dans le code — contraire à la consigne "aucune donnée inventée". Documenté en commentaire dans `index.js` à l'endroit exact où cette étape aurait dû se trouver, pour qu'un futur lot qui ajouterait un vrai chargement plugin au boot sache où l'accrocher.
  - **Registre des jobs planifiés (`registerWorker()`/`recordWorkerRun()`, nouveau dans `startupStatusService.js`)** : chaque service de planification déjà existant (`backupService`, `statusHistoryService`, `hostMetricsService`, `infraLoadService`, `vaultRotationService`, `secretLeakScanService`, `scheduledTrivyScanService`, `kubernetesAlertService`, `previewEnvironmentCleanupService`, `wazuhAlertNotificationService` — dont le job Wazuh du Lot D7 et le nettoyage de previews) s'auto-enregistre à l'import avec son intervalle RÉEL (lu directement dans la constante `*_INTERVAL_MS` du fichier, ou `intervalHint` textuel pour les deux jobs "quotidien à heure fixe" — sauvegardes 3h, scan de fuite de secrets 4h, qui n'ont pas d'intervalle constant en ms) et rapporte chaque exécution réelle (`ok`/`error`) via `recordWorkerRun()`. Aucun scheduler inventé : uniquement les 10 jobs réellement câblés dans `index.js`. `lastRunAt`/`nextRunEstimateAt` restent `null` tant qu'aucun cycle n'a eu lieu depuis CE démarrage (honnête — rien n'est persisté entre redémarrages, ce n'est pas un historique).
  - **Endpoints** : `GET /api/system/status/startup` (admin, existant, enrichi avec `degraded`/`failedAtStep`/`detail` par étape) ; nouveau `GET /api/system/status/runtime` (admin) — état process backend (uptime, PID, version Node), accessibilité frontend (requête sortante réelle vers `FRONTEND_ORIGIN`, limite documentée en commentaire : en dev le frontend Vite tourne sur un process séparé, un échec de cette requête sortante ne prouve pas qu'un navigateur ne peut pas l'atteindre), et la liste des workers. **Accès pré-authentification** : ajouté un nouveau `GET /api/status/startup` (public, dans `status.routes.js`, avant `router.use(requireAuth)` — même emplacement que la sonde `/status/health` déjà publique) qui expose une vue volontairement réduite (nom d'étape + statut + durée uniquement, **jamais le message d'erreur détaillé** qui peut contenir un hôte/chemin interne) : c'était réalisable proprement car ce routeur avait déjà un précédent d'endpoint public juste au-dessus, contrairement à `system.routes.js` qui applique `requireAuth`+`requireRole('admin')` en bloc sur tout le routeur — pas de hack sur le middleware d'auth, juste un nouvel endpoint minimal au bon endroit.
  - **Frontend** : nouvelle page `frontend/src/pages/System/StartupStatusPage.jsx` (+ CSS), route `/system/startup` dans `App.jsx` (protégée par `RequirePermission domain="settings" level="admin"`, cohérent avec le reste des pages d'administration système — pas de contournement d'auth côté frontend puisque l'API `/system/status/*` reste elle-même admin-only, seule l'API `/status/startup` réduite est publique). Affiche : KPI d'état global (Prêt/Dégradé/En échec + étape d'échec), timeline des étapes avec durée/statut/détail, bloc runtime (backend/frontend), tableau des workers avec dernière/prochaine exécution. Polling 15s (démarrage) / 10s (runtime) via `useApi(..., { pollMs })` déjà existant, pas de nouveau mécanisme de polling créé. Lien d'accès ajouté dans `Paramètres → Système` (`SystemPanel.jsx`, nouveau panneau en tête avec lien vers `/system/startup`) — pas de nouvel élément dans la barre de navigation principale, cohérent avec le fait que "Système" est déjà un sous-onglet des Paramètres pour ce type de page.
  - **Diagnostic panne démarrage** : `getStartupStatus()` calcule désormais `failedAtStep` (nom de la première étape `failed`) et `degraded` (vrai si au moins une étape est `degraded` sans échec bloquant) — affichés en tête de l'écran. Testé réellement : avec Postgres/intégrations partiellement configurées dans cet environnement, `integrationsCheck` est passé `degraded` avec le détail exact `"3/4 intégration(s) configurée(s) en échec: kubernetes, haproxy, certManager"`, visible à la fois via l'API et sur l'écran.
  - **Limite honnête — scénario de panne DB réelle non testé en isolation.** Consigne explicite de ne jamais toucher au process/port de l'instance de dev partagée. Cet environnement n'avait en réalité AUCUNE instance backend déjà lancée au moment de ce lot (`lsof` sur le port 4000 : rien en écoute) — un backend a donc été démarré ponctuellement pour la vérification Playwright (voir ci-dessous) puis arrêté proprement à la fin (`pkill` du process lancé par cet agent uniquement, vérifié par un nouveau `curl` en échec de connexion après coup). Lancer une DEUXIÈME instance avec une `DATABASE_URL` volontairement invalide en parallèle aurait nécessité un port distinct et n'a pas été fait par prudence (temps disponible + risque de confusion avec le process de test déjà en cours) : le comportement de l'étape `dbConnection` en cas d'échec réel (elle appelle `runStep()`, donc un rejet de `pool.connect()` ferait échouer tout le démarrage avec `failedAtStep: "dbConnection"` et le message d'erreur réel de `pg`) est garanti par la lecture du code (`runStep` propage l'erreur, `beginStep().fail()` est appelé, le process ne devient jamais `ready`), mais n'a pas été observé en conditions réelles dans cette session.
  - **Vérifié réellement** : `node --check` sur les 13 fichiers backend modifiés/créés — tous OK. `node --test` : **133 pass / 4 fail / 3 skipped (140 total) avant et après ce lot** — mêmes 4 échecs préexistants (non liés à ce lot), aucune régression. `npx vite build` sans erreur (496 modules). Un backend a été démarré ponctuellement (aucune instance n'était déjà active) : `GET /api/status/startup` (public) et `GET /api/system/status/startup`/`GET /api/system/status/runtime` (admin, session curl réelle `admin@homelab.local`/`admin1234`) confirmés avec des durées non nulles et variées (1ms à 17ms selon l'étape, jamais un `0ms` uniforme suspect). Vérifié en direct via Playwright : connexion réussie, navigation vers `/system/startup`, timeline affichée avec les 9 étapes réelles (durées, `integrationsCheck` en badge "Dégradé" avec le détail exact), bloc runtime backend (PID, uptime, version Node réels) et frontend (latence 3-6ms, code HTTP 200 réel), tableau des 10 workers avec intervalles réels et `lastRunAt`/`runCount` incrémentés en direct sur les jobs à intervalle court (30s-60s) déjà déclenchés une fois pendant la session de test. Capture d'écran prise dans `.playwright-cli/` (jamais à la racine du repo, conforme au `.gitignore`). Backend de test arrêté proprement à la fin, aucune instance restée en arrière-plan.
  - **Fichiers** : `backend/src/services/startupStatusService.js` (étendu : `degraded()`, `registerWorker`/`recordWorkerRun`/`getWorkersStatus`, `getProcessRuntimeStatus`, `failedAtStep`/`degraded` calculés), `backend/src/index.js` (nouvelles étapes `configLoad`/`dbConnection`/`integrationsCheck`/`certificatesCheck`/`networkCheck`, `runOptionalStep()`), `backend/src/routes/system.routes.js` (nouveau `GET /status/runtime`), `backend/src/routes/status.routes.js` (nouveau `GET /startup` public réduit), `backend/src/services/backupService.js`, `backend/src/services/statusHistoryService.js`, `backend/src/services/hostMetricsService.js`, `backend/src/services/infraLoadService.js`, `backend/src/services/vaultRotationService.js`, `backend/src/services/secretLeakScanService.js`, `backend/src/services/scheduledTrivyScanService.js`, `backend/src/services/kubernetesAlertService.js`, `backend/src/services/previewEnvironmentCleanupService.js`, `backend/src/services/wazuhAlertNotificationService.js` (chacun : `registerWorker()` à l'import + `recordWorkerRun()` dans son cycle planifié), `frontend/src/pages/System/StartupStatusPage.jsx` (nouveau), `frontend/src/pages/System/StartupStatusPage.css` (nouveau), `frontend/src/App.jsx` (route `/system/startup`), `frontend/src/pages/Settings/SystemPanel.jsx` (lien d'accès).

## Lot D10 — Création de projet facilitée + liaison de dépôts multiples (2026-08-23)

- [x] **Écart réel constaté (audit avant toute modification) : très faible — le flux était déjà globalement bon.**
  - **Création de projet (`POST /projects`, `backend/src/routes/projects.routes.js:163`)** : déjà minimaliste — seul `name` est requis (`if (!name) return res.status(400)...`), tous les autres champs (description, tags, membres, icône, couleur) sont optionnels. Le formulaire `ProjectsPage.jsx` (modale "Nouveau projet") n'a que deux champs visibles par défaut (Nom, Description) plus des pickers optionnels icône/couleur/membres — pas un formulaire à 15 champs obligatoires. Les environnements `production`/`staging` sont déjà auto-provisionnés côté relationnel (`orgStore.createProject` + logique existante, confirmée dans le commentaire du code : "deux environnements (production, staging) créés automatiquement"). **Aucune régression de complexité à corriger ici** — le formulaire de création n'a donc pas été modifié en profondeur, seul du texte serait à améliorer et ça n'apportait rien de mesurable.
  - **Liaison de dépôts existants** : le modèle de données confirme le many-to-many attendu — `repoKeys` est un tableau STOCKÉ PAR PROJET (`store.createProject({..., repoKeys})`, `PUT /projects/:id` accepte un nouveau tableau complet). Rien n'empêche qu'une même `repoKey` apparaisse dans le `repoKeys` de plusieurs projets différents : chaque projet a son propre tableau indépendant, il n'existe aucune contrainte d'unicité/exclusivité au niveau store ou route. Un dépôt peut donc déjà, au niveau données, être rattaché à N projets. Côté UI, un composant `RepoPicker` existait déjà dans `ProjectDetailPage.jsx` (panneau "Dépôts rattachés") sous forme de modale à cases à cocher listant TOUS les dépôts disponibles — ce n'était donc déjà PAS un flux "un dépôt à la fois page par page" comme le message utilisateur le redoutait. La vraie limite trouvée : chaque case cochée déclenchait immédiatement un `PUT /projects/:id` séparé (un appel réseau par clic, pas de validation groupée) et il n'y avait pas de champ de recherche pour filtrer la liste si elle devient longue.
- [x] **Ce qui a été fait (amélioration UX ciblée, pas de changement de modèle de données — inutile puisque déjà many-to-many)** :
  - `RepoPicker` (`frontend/src/pages/Deployments/ProjectDetailPage.jsx`) réécrit : sélection LOCALE en attente (`pending`, initialisée sur les dépôts déjà liés à l'ouverture), un champ de recherche (`Rechercher un dépôt…`) filtrant la liste par nom, et un unique bouton "Enregistrer (N rattaché·s)" qui envoie tout le tableau en un seul `PUT /projects/:id` au lieu d'un appel par case cochée — c'est la "façon de lier RAPIDEMENT plusieurs dépôts en une seule action" demandée. Le bouton reste désactivé tant qu'aucune modification n'a été faite (`dirty`). `toggleRepo` a été remplacé par `saveRepoKeys(repoKeys)` dans le composant parent, avec une notification qui indique le delta réel de dépôts rattachés (`+N dépôt(s) rattaché(s)`) plutôt qu'un message générique.
  - CSS : classe `.pd-picker-search` ajoutée dans `ProjectDetailPage.css`.
  - Formulaire de création de projet (`ProjectsPage.jsx`) : non modifié — vérifié qu'il était déjà conforme à la demande ("nom + description minimum, tout le reste configurable après").
- [x] **Vérifié réellement** : `node --check` sur les fichiers backend touchés (aucun backend modifié dans ce lot — seul le frontend a changé, donc pas de `node --check` applicable ; `node --test` relancé quand même pour vérifier l'absence de régression). `node --test` : **133 pass / 4 fail / 3 skipped (140 total)** — identique au plancher attendu, mêmes 4 échecs préexistants. `npx vite build` : succès (496 modules, aucune erreur). Vérifié en direct via Playwright (`admin@homelab.local`/`admin1234`, backend démarré ponctuellement car aucune instance n'était déjà active sur le port 4000 au moment du test) : création d'un projet réel "D10 Test Projet" avec uniquement le champ Nom rempli (flux minimal confirmé, notification "D10 Test Projet créé" reçue, projet visible dans la liste avec `Aucune description` honnête) ; ouverture du panneau "Dépôts rattachés" → onglet "Code" de deux projets différents (le nouveau et `api-gateway` existant) → modale "Rattacher des dépôts" avec le nouveau sous-titre "Cochez un ou plusieurs dépôts existants, puis validez en une fois" affichée correctement.
- [x] **Limite honnête — le test bout-en-bout du multi-rattachement réel n'a pas pu être exercé.** La page `/deployments/repos` de cet environnement affiche "Aucune forge configurée (GitLab/GitHub) — voir Paramètres → Intégrations" (0 dépôt au total, confirmé aussi dans la modale du nouveau `RepoPicker` : "Aucun dépôt disponible"), donc il n'existe aucun dépôt réel catalogué dans cette instance de dev pour cocher plusieurs cases et valider en une fois. Le comportement du composant (sélection multiple en attente, filtre de recherche, sauvegarde groupée en un seul PUT) a été vérifié par lecture de code et par test d'ouverture/fermeture de la modale, mais pas par un scénario complet "coche 3 dépôts → Enregistrer → les 3 apparaissent dans deux projets différents" faute de données réelles disponibles — conforme à la consigne de ne jamais inventer de dépôt de test. Un futur lot qui connecterait une forge GitHub/GitLab réelle à cet environnement permettrait de compléter cette vérification.
- **Fichiers modifiés** : `frontend/src/pages/Deployments/ProjectDetailPage.jsx` (`RepoPicker` réécrit avec recherche + sélection groupée, `toggleRepo` remplacé par `saveRepoKeys`), `frontend/src/pages/Deployments/ProjectDetailPage.css` (`.pd-picker-search`).

## Lot D11 — Actions CI/CD proposées par technologie détectée, depuis la fiche projet (2026-08-23)

- [x] **Investigation préalable : où `buildCiWorkflow()` (Lot D6) était déjà déclenchable.** Un seul point d'entrée UI existait : `GenerateCiModal` dans `frontend/src/pages/Deployments/GitReposPage.jsx` (page globale "Code" / dépôts), qui appelle `POST /repos/:key/workflows/generate-ci` avec `baseBranch: repo.defaultBranch`. Cette route (`backend/src/routes/repos.routes.js:298`) lit l'arborescence réelle du dépôt, appelle `buildCiWorkflow()`, ouvre une VRAIE branche + commit + pull request GitHub (`github.createBranch`/`commitFile`/`createPullRequest`) et retourne `{ branch, pullRequest: { number, webUrl } }` — confirmé par lecture du code, ce n'est jamais un succès simulé. Elle est **restreinte à `provider === 'github'`** (400 explicite sinon — GitLab garde son `.gitlab-ci.yml` natif équivalent, commentaire du code). La route est seulement protégée par `requireAuth` (pas de rôle admin spécifique), donc l'appeler directement depuis la fiche projet n'introduit aucun contournement de permission. Avant ce lot, il fallait quitter la fiche projet et naviguer vers `/deployments/repos` pour la déclencher — aucune trace de cette action dans `ProjectDetailPage.jsx`, confirmant le besoin exprimé ("directement dans les projets").
  - Route associée `GET /repos/:key/structure?ref=<branche>` (déjà existante, utilisée par `RepoStructureModal` de `GitReposPage.jsx`) : renvoie `structure.stack` (liste des technologies réellement détectées via signaux de fichiers + frameworks Node lus dans `package.json`) et `structure.hasCI` (vrai si un workflow `.github/workflows` ou `.gitlab-ci.yml` existe déjà) — support GitHub ET GitLab, contrairement à la génération elle-même.
- [x] **Ce qui a été fait, dans `ProjectDetailPage.jsx` (panneau "Activité des dépôts", déjà présent depuis un lot antérieur, span=12, un dépôt rattaché = une carte)** :
  - Nouveau hook `useRepoStacks(repos)` : pour chaque dépôt rattaché au projet (hors ceux déjà en erreur), appelle `GET /repos/:key/structure?ref=<defaultBranch>` une fois (pas de polling), stocke `{ loading, structure, error }` par clé de dépôt. Jamais de stack devinée depuis le nom du dépôt — uniquement le résultat réel de la détection backend du Lot D6.
  - Nouveau composant `RepoCiRow` (une ligne ajoutée sous les commits de chaque carte dépôt) : affiche la stack détectée en badges réels (`stack.length === 0` → "Stack non reconnue", jamais une valeur par défaut inventée), un badge "CI déjà présente" si `hasCI` est vrai, et l'action de génération :
    - GitHub + stack non vide → bouton "Générer le pipeline CI/CD" → `POST /repos/:key/workflows/generate-ci` (même route/service que Lot D6) → en cas de succès, remplace le bouton par un lien direct "Voir la pull request #N" (`result.pr.webUrl`) ; en cas d'échec (ex. permission d'écriture insuffisante côté GitHub, forge mal configurée), affiche un badge rouge avec le message d'erreur RÉEL renvoyé par l'API, jamais un succès de façade.
    - GitHub sans stack détectée → "Aucune stack reconnue à automatiser" (pas de bouton qui produirait un job générique inutile).
    - GitLab → "CI native GitLab" (pas de bouton, car la route backend refuse explicitement ce provider — cohérent avec le comportement déjà existant de `GitReposPage.jsx`).
  - **Action groupée "Tout brancher d'un coup"** : bouton dans l'en-tête du panneau ("Générer les pipelines pour tous les dépôts (N)"), affiché uniquement si au moins 2 dépôts du projet sont éligibles (GitHub + stack détectée). `generateAllCi()` boucle **séquentiellement** (pas `Promise.all`, pour ne pas saturer l'API GitHub et afficher chaque résultat au fur et à mesure) sur les dépôts éligibles et appelle `generateCi(r)` pour chacun — **chaque dépôt garde sa propre stack détectée individuellement** (le hook `useRepoStacks` interroge `/structure` par dépôt, jamais une stack unique appliquée à tous), conforme à la demande explicite de ne pas mélanger un frontend et un backend sous une même détection.
  - CSS : `.pd-repo-ci-row`, `.pd-repo-stack-badge` ajoutées dans `ProjectDetailPage.css`.
- [x] **Vérifié réellement** : `node --check backend/src/routes/repos.routes.js` OK (aucun fichier backend modifié dans ce lot — uniquement du frontend, la route/service du Lot D6 étant réutilisée telle quelle). `node --test` (suite complète backend) : **133 pass / 4 fail / 3 skipped (140 total)** — identique au plancher documenté, mêmes 4 échecs préexistants non liés à ce lot, aucune régression. `npx vite build` : succès (496 modules, aucune erreur). Vérifié en direct via Playwright (`admin@homelab.local`/`admin1234`, backend démarré ponctuellement car aucune instance n'était déjà active sur le port 4000, arrêté proprement à la fin) : connexion réussie, navigation vers la fiche projet "D10 Test Projet" (créée lors du Lot D10, aucun dépôt rattaché), onglet "Code" → panneaux "Dépôts rattachés" et "Activité des dépôts" affichent tous deux l'état vide honnête ("Aucun dépôt rattaché" / "Aucun dépôt rattaché à ce projet.") et **aucun bouton d'action groupée n'apparaît** (conditionné correctement à `eligibleCount > 1`, qui vaut 0 ici). Aucune erreur console (0 erreur/0 avertissement en dehors du message informatif React DevTools).
- [x] **Limite honnête — aucun test bout-en-bout complet de la génération réelle de pipeline.** Comme documenté dans le Lot D10, cet environnement de dev n'a **aucune forge configurée** (`GET /api/repos` renvoie `{ok:true, items:[]}`) et donc aucun dépôt GitHub/GitLab réel catalogué à rattacher à un projet. Il n'a donc pas été possible d'exercer par Playwright : l'affichage réel d'une stack détectée en badges, le clic sur "Générer le pipeline CI/CD" avec ouverture d'une vraie pull request, l'action groupée sur plusieurs dépôts avec des stacks différentes (ex. frontend React + backend Java), ni un cas d'échec réel (permission GitHub insuffisante). Le comportement de `useRepoStacks`/`RepoCiRow`/`generateAllCi` a été vérifié par lecture de code et par la cohérence de l'état vide (rendu sans erreur, action groupée bien masquée), en réutilisant strictement les routes/service déjà validés dans le Lot D6 (`buildCiWorkflow()`) et dans `GitReposPage.jsx` (même appel `POST .../workflows/generate-ci`, mêmes champs de réponse). Une vérification bout-en-bout complète nécessite qu'un futur lot connecte une vraie intégration GitHub à cet environnement, avec au moins deux dépôts de stacks différentes rattachés à un même projet.
- **Fichiers modifiés** : `frontend/src/pages/Deployments/ProjectDetailPage.jsx` (nouveau hook `useRepoStacks`, nouveau composant `RepoCiRow`, `RepoActivityPanel` étendu avec génération CI par dépôt + action groupée), `frontend/src/pages/Deployments/ProjectDetailPage.css` (`.pd-repo-ci-row`, `.pd-repo-stack-badge`). Aucun fichier backend modifié — routes/service du Lot D6 réutilisés tels quels.

- [x] **Lot D12 — Responsive toutes tailles (320px → 3840px, ultra-wide, portrait, zoom navigateur)**, 2026-08-23. Demande : la plateforme doit s'adapter à toutes les tailles/formats d'écran, pas seulement mobile — 320/375/390/430/768/1024/1280/1440/1920/2560/3440/3840px, ultrawide, écrans très hauts, zoom 100/125/150/200%, aucun scroll horizontal global, sidebar stable, tableaux scrollables localement, modales adaptées, format portrait. **Méthode d'audit** : backend + frontend Vite lancés en local (`admin@homelab.local`), balayage automatisé via `mcp__playwright__browser_run_code_unsafe` (boucle unique naviguant sur 13 routes représentatives — Vue générale, Mon travail [via /deployments/my-work], Kubernetes, Réseaux/Topologie [Lot C1], Réseaux/Proxies, Projets, fiche Projet, Paramètres, Catalogue, Déploiements/Releases, Dépôts Git, Tests & qualité, Pipelines — × 17 largeurs de 320 à 3840px + 3 formats portrait 430×932/768×1024/1024×1366), mesurant à chaque combinaison `document.documentElement.scrollWidth`/`body.scrollWidth` vs `window.innerWidth` (détection de tout scroll horizontal global) et la position du bloc d'actions du header (`.header-actions.getBoundingClientRect().right` vs viewport, pour détecter un débordement *clippé* invisible à la métrique précédente). Le zoom navigateur réel n'a pas pu être piloté par Playwright (pas d'API zoom CSS exposée côté MCP) : **testé par proxy** en réduisant le viewport CSS dans les mêmes proportions qu'un zoom (ex. 200% ≈ viewport ÷2, table `100%→1280×800, 125%→1024×640, 150%→853×533, 200%→640×400`), ce qui exerce les mêmes points de rupture CSS qu'un vrai zoom (le viewport CSS effectif rétrécit de la même façon dans les deux cas) mais ne vérifie pas les artefacts propres au rendu zoomé du navigateur lui-même (netteté, `devicePixelRatio`) — limite assumée, à revérifier manuellement si un doute subsiste.

  **2 régressions réelles trouvées et corrigées, toutes dans `frontend/src/styles/global.css`** (aucune autre page auditée n'a montré de scroll horizontal global ni de débordement, à aucune des tailles testées) :

  1. **Header clippé sous ~620px (icônes actions inatteignables)**. À 320-480px, le bloc marque (`header-brand` : logo + "Nexus Console" + "homelab.local", ~134px) n'était réduit qu'au sous-titre (`header-brand-sub`, caché sous 560px) — le nom complet restait affiché. Combiné à la barre de recherche pleine largeur (220px, ne se réduisait en icône qu'en dessous de 480px), le total marque + recherche + 3 icônes (thème/notifications/compte) dépassait la largeur disponible du header. `document.scrollWidth` ne le détectait pas (pas de scroll de page) car `.header-bar` masque son propre dépassement plutôt que de le laisser pousser le document — mais l'icône compte (avatar) se retrouvait positionnée hors du viewport, donc invisible et non cliquable. Mesuré précisément via `getBoundingClientRect()` : à 320px l'icône compte finissait à x=379.7 (viewport=320, débordement de 59.7px) ; entre 481 et 619px (recherche encore pleine largeur, seuil de repli à 480px trop bas), l'icône compte débordait encore de 18 à 97px selon la largeur. **Correctif** : seuil de repli de la recherche pleine largeur → icône relevé de 480px à 620px (calculé pour couvrir la pire largeur mesurée avec marge) ; nouveau point de rupture à 400px qui masque entièrement le texte de la marque (`.header-brand-text`, ne laisse que le logo) et resserre les espacements du header (`gap`/`padding`), pour libérer la place nécessaire aux 320-399px les plus étroits. Revérifié par le même balayage automatisé (320 à 3840px + les seuils exacts 400/401/480/481/600/619/620/621) sur les 8 routes principales : plus aucun débordement, l'icône compte reste dans le viewport à toutes les tailles.
  2. **Tableau large qui fait défiler toute la colonne de contenu au lieu de lui seul, sous 860px** (`/deployments/releases` notamment, tableau "Applications suivies" à 896px de large). Cause racine identifiée précisément par inspection de la chaîne d'ancêtres (`getComputedStyle` + `getBoundingClientRect` remontés depuis `<table>`) : sous 860px, `.dev-layout` (et de façon identique `.k8s-layout`/`.network-layout`) passe en `flex-direction: column` (bandeau de sous-nav horizontal au-dessus du contenu, déjà documenté comme correctif mobile) mais conservait `align-items: flex-start` hérité de la règle desktop. Or en colonne flex, `align-items` régit l'axe transversal — c'est-à-dire la **largeur** — et `flex-start` désactive l'étirement par défaut (`stretch`) : `.dev-layout-content` (et son tableau) se dimensionnait alors sur son contenu intrinsèque (898px) au lieu de la largeur réellement disponible (309px à 375px de large), débordant silencieusement de tout le conteneur `.dev-layout` (`overflow-x: visible`) jusqu'à ce que `.shell-main` (qui a lui `overflow-x: auto`) absorbe le débordement — résultat : faire défiler la table faisait défiler **toute la page** (bandeau de sous-nav + titre compris), pas seulement le tableau, alors que le conteneur `overflow-x: auto` dédié au tableau (`table > div`, déjà en place) était bien présent mais ne s'activait jamais faute d'une largeur de référence correcte à lui transmettre. **Correctif** : ajout de `align-items: stretch` dans les 2 règles `@media (max-width: 860px)` qui passent `.dev-layout`/`.k8s-layout`/`.network-layout` en colonne. Revérifié : à 375px, la chaîne d'ancêtres montre maintenant `.dev-layout-content` correctement limité à 309px de large, et c'est bien le `div` immédiat autour de `<table>` qui absorbe le débordement (`scrollWidth: 583` / `clientWidth: 307`, scroll local fonctionnel testé en direct via `wrap.scrollLeft = 200`) — page, sous-nav et titre restent fixes.

  **Confirmé déjà correct, sans modification nécessaire** :
  - **Ultra-wide (1920 → 3840px)** : `.shell-route-page` (`frontend/src/components/layout/Shell.css`) limite déjà tout le contenu routé à `max-width: 1480px` centré (`margin: 0 auto`), ce qui évite à la fois l'étirement infini et l'effet "texte perdu" sur un écran 3840px — vérifié visuellement par capture d'écran sur la fiche projet à 3840px (colonne de contenu bien centrée dans l'espace disponible après la barre de navigation). Aucun changement nécessaire sur ce point : le choix architectural déjà en place (Groupe A/B antérieur) répond exactement à l'exigence "ni étiré à l'infini, ni trois mots perdus".
  - **Modales** : `frontend/src/components/ui/Modal.jsx`/`Modal.css` déjà robustes à toutes les tailles — `.modal-overlay` centre en flex avec `padding: 20px` (jamais collé aux bords même à 320px), `.modal-card` en `width:100%` plafonné par un `maxWidth` par instance et `max-height: 86vh` avec corps en `overflow: auto` (jamais plus grand que l'écran). Vérifié visuellement à 320px sur la modale "Nouveau projet" (`ProjectsPage.jsx`) : bien centrée, bouton fermer/annuler/valider tous accessibles, aucun débordement. Seul défaut mineur observé et **non corrigé** (jugé hors périmètre — cosmétique, pas fonctionnel) : dans cette modale précise, la rangée Nom/Description en `flex` (`.projects-form-row`, `ProjectsPage.css`) réduit chaque champ à ~90px à 320px, tronquant le texte de substitution affiché (`text-overflow: ellipsis` implicite du champ) — les deux champs restent pleinement utilisables (saisie, curseur, valeur réelle non tronquée à l'usage), seul l'aperçu visuel est serré.
  - **Sidebar/nav** (Lot A2) : rail de navigation principal (`DomainNav`) et tiroir mobile (<860px) confirmés stables à toutes les largeurs testées, y compris les 3 formats portrait (430×932, 768×1024, 1024×1366) — aucune régression du comportement sticky/scroll documenté au Lot A2.
  - **Topologie réseau (Lot C1, react-flow)** : page `/network` vérifiée à 3840px — aucune donnée réseau configurée dans cet environnement de dev (aucun proxy/HAProxy/Traefik/Kubernetes/Argo CD/Proxmox réel), donc le canvas react-flow lui-même ne s'est jamais rendu (seul l'état vide honnête "Configurez au moins un proxy..." s'affiche, lui-même correctement centré/limité par `.shell-route-page`). **Limite honnête** : le comportement réel du canvas graphique (nœuds/arêtes, zoom interne react-flow, redimensionnement du viewport SVG) en ultra-wide et sous zoom navigateur n'a donc pas pu être vérifié visuellement faute de données réelles à afficher — seul le conteneur qui l'héberge (`TopologyPage.jsx`, mêmes classes de layout que le reste de l'app) a été confirmé sans débordement.
  - **Tableaux ailleurs dans l'app** (`/deployments/repos`, `/network/proxies`, `/deployments/tests`) : aucun autre tableau volumineux trouvé dans les routes testées avec assez de données pour dépasser la largeur de son conteneur dans cet environnement de dev (peu de lignes/colonnes réelles) — seul `/deployments/releases` (colonnes NOM/SOURCE GIT/APPLICATION ARGO/etc., 3 applications réelles suivies) a révélé le bug n°2 ci-dessus, désormais corrigé et vérifié comme un correctif générique (les 3 layouts à sous-nav partagent la même règle CSS).

  **Vérifié réellement** : `npx vite build` (496 modules, aucune erreur). `node --test` (suite backend, non touchée par ce lot — uniquement du CSS frontend) : **133 pass / 4 fail / 3 skipped (140 total)**, identique au plancher documenté, les 4 échecs sont les mêmes que d'habitude (`restoreBackup`/`createBackup` sans Postgres, `enqueue`/`getJob` sans `DATABASE_URL` — préexistants, non liés à ce lot). Balayage Playwright automatisé (voir méthode ci-dessus) exécuté **avant** et **après** chaque correctif pour confirmer la régression puis sa résolution, sur 13 routes × 17 largeurs + 3 portraits + 4 tailles de zoom-proxy sur 4 routes clés — aucun débordement restant nulle part après correctifs. Capture d'écran visuelle à 320px (header, tableau, modale) et 3840px (fiche projet, topologie) pour confirmation humaine en plus des métriques automatisées.
  **Fichiers modifiés** : `frontend/src/styles/global.css` uniquement (seuil de repli recherche/marque du header relevé et affiné avec un nouveau point de rupture 400px ; `align-items: stretch` ajouté aux 2 media queries `max-width: 860px` qui empilent `.dev-layout`/`.k8s-layout`/`.network-layout`). Aucun fichier backend touché. Aucune logique métier modifiée — strictement CSS/layout.
  **Limites documentées** : zoom navigateur testé par proxy (viewport réduit dans les mêmes proportions), pas par un vrai contrôle de zoom CSS/OS — voir méthode ci-dessus ; canvas react-flow de la Topologie non vérifiable visuellement faute de données réseau réelles dans cet environnement ; troncature cosmétique mineure non corrigée dans la modale "Nouveau projet" à 320px (champs toujours fonctionnels).

- [x] **Lot D13 — Page de surveillance en boucle (« Mur de surveillance », `/monitor-wall`)**, 2026-08-23. Demande : une page dédiée à laisser affichée en continu sur un écran (type dashboard mural) pour surveiller commits/push, pipelines et alertes, sans interaction requise, avec auto-refresh et une lisibilité de loin.
  **Ce qui a été fait** : nouvelle page `frontend/src/pages/Deployments/MonitorWallPage.jsx` + `MonitorWallPage.css`, route `/monitor-wall` (niveau racine, à côté de `/manual`/`/report`, protégée uniquement par `RequireAuth` du Shell — aucune restriction de rôle supplémentaire nécessaire, c'est une vue en lecture seule agrégeant des données déjà accessibles ailleurs). **Aucune nouvelle route backend** : la page réutilise strictement l'existant — `GET /repos` (liste des dépôts), `GET /repos/:key/commits` (Repository Workspace, Lot précédent — un appel par dépôt, plafonné à 8 dépôts triés par activité récente pour éviter une explosion du nombre d'appels), `GET /pipelines/runs` (agrégé GitLab+GitHub, Lot D6/PipelinesPage), `GET /wazuh/alerts` + `GET /wazuh/summary` (Lot D7) et `GET /{gitlab,github,gitea}/status` (badges d'intégration, Lot A5). Contenu affiché : 4 KPI en très grands chiffres (dépôts connectés, pipelines en cours, échecs 24h, alertes actives), 3 colonnes (derniers commits/push tous dépôts confondus triés par date, derniers pipelines CI/CD avec statut coloré, alertes de sécurité Wazuh/agents déconnectés), footer avec badges d'état des 3 forges. Auto-refresh toutes les 20s en réutilisant le pattern déjà en place ailleurs (polling `setInterval` + `markBackground()` avant les appels silencieux, même mécanisme que `useApi({pollMs})` et `OpenAlertsPanel.jsx`, pas de nouveau mécanisme inventé) ; horloge affichée mise à jour chaque seconde pour rendre visible que la page est vivante sur un écran mural. Pas de pagination : chaque section plafonne aux N plus récents (12 commits, 10 pipelines, 8 alertes) avec défilement vertical local si la liste dépasse la hauteur de la carte — pas de clic requis. Mise en page pensée écran mural : polices nettement plus grandes que le reste de l'app (KPI jusqu'à 48px sur très grand écran ≥1800px via media query dédiée), fort contraste (couleurs de tonalité `--tone-ok/warn/crit` du design system), 3 colonnes qui repassent à 1 sous 1100px (cohérent avec le responsive du Lot D12, aucun scroll horizontal introduit). Lien d'accès ajouté dans le header (`Header.jsx`, icône `gauge`, à côté du lien Manuel) — visible sur toutes les pages, pas caché dans un sous-menu, pensé pour être ouvert dans un onglet séparé/plein écran sur un second moniteur.
  **Vérifié réellement** : `npx vite build` (498 modules, aucune erreur). `node --test` (aucun fichier backend touché dans ce lot) : **133 pass / 4 fail / 3 skipped (140 total)**, identique au plancher documenté, mêmes 4 échecs préexistants non liés à ce lot. Vérifié en direct via Playwright (`admin@homelab.local`/`admin1234`, backend+frontend déjà actifs en local) : connexion réussie, lien "Mur de surveillance" visible et cliquable dans le header, page `/monitor-wall` se charge sans erreur console bloquante, affiche l'état honnête attendu dans cet environnement de dev sans forge configurée (`GET /repos` → `{items:[]}`) : "0" sur les 4 KPI, "Aucune forge configurée (GitLab/GitHub/Gitea) — voir Paramètres → Intégrations" pour commits et pipelines, "Aucune alerte ouverte" pour la sécurité (le 409 renvoyé par `/wazuh/summary`, Wazuh non configuré, est absorbé proprement par `Promise.allSettled` sans casser l'affichage), les 3 badges de forge affichent "non configuré". Auto-refresh confirmé objectivement via `playwright-cli requests` : un second lot complet des 7 appels (`/repos`, `/pipelines/runs`, `/wazuh/alerts`, `/wazuh/summary`, `/gitlab/status`, `/github/status`, `/gitea/status`) observé ~20s après le premier, sans rechargement de page. Capture d'écran visuelle confirmant la lisibilité forte (grands chiffres verts/blancs sur fond sombre, cartes bien séparées).
  **Fichiers modifiés/créés** : `frontend/src/pages/Deployments/MonitorWallPage.jsx` (nouveau), `frontend/src/pages/Deployments/MonitorWallPage.css` (nouveau), `frontend/src/App.jsx` (route), `frontend/src/components/layout/Header.jsx` (lien d'accès). Aucun fichier backend touché.
  **Limites documentées** : comme pour tous les lots CI/CD/dépôts précédents, cet environnement de dev n'a aucune forge (GitLab/GitHub/Gitea) ni agent Wazuh réellement configurés — impossible donc de vérifier visuellement le rendu avec de vrais commits/pipelines/alertes qui s'affichent (seul l'état vide honnête a pu être exercé). Le plafond de 8 dépôts pour les commits est un compromis pour éviter un nombre d'appels HTTP non borné côté frontend (pas d'endpoint agrégé "derniers commits tous dépôts" côté backend) — resterait pertinent d'ajouter un tel endpoint si le nombre de dépôts réels dépasse largement 8 en usage réel. Le défilement automatique (auto-scroll) n'a pas été implémenté : les listes sont plafonnées à un nombre fixe d'éléments qui tiennent dans la hauteur de carte plutôt que de faire défiler tout l'historique — choix jugé plus lisible pour un écran mural (pas de texte qui bouge tout seul, plus fatiguant à lire de loin) mais différent d'un "carrousel" si c'est ce qui était attendu.
