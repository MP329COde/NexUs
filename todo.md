...
- [~] CSS (en cours, page par page) : fait — `HomePage`, `NetworkPage`, `KubernetesPage`, `StoragePage`, `SecurityPage`, `MonitoringPage`, `TopologyPage`, `HAProxyPage`, `CertificatesPage`, `FirewallPage` (10 pages, 7 commits séparés). Restant : `Deployments/*` (le plus gros lot), `Settings/*`, `Infrastructure/*`, `Manual`, `Report`, `Account` — à poursuivre page par page.
