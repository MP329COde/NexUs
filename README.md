...
- Rate-limiting sur `/api/terminal`, `/api/kubernetes` et `/api/proxmox` (30 req/min, `strictLimiter`) :
  ...
  uniquement le terminal aurait laissé la porte ouverte par les routes Kubernetes/Proxmox directes
  qu'il encapsule. Vérifié en conditions réelles sur les trois : 429 après la ~29ᵉ requête en une minute.
