...
- **WikiPage.jsx** — Wiki d'équipe par organisation : liste/recherche de pages, édition (titre + texte), historique des révisions, suppression. Socle PostgreSQL (`GET/POST/PUT/DELETE /api/wiki`). **Déplacé dans le menu de l'organisation** (`/deployments/organizations/:id/wiki`, accessible depuis `OrganizationDetailPage.jsx`) — n'apparaît plus dans la barre latérale Développement ni derrière un sélecteur libre : le wiki appartient toujours à une organisation précise, consultée depuis sa fiche.
- **OrganizationDetailPage.jsx** — Fiche d'une organisation (nouveau) : menu (Wiki d'équipe, Membres), liste des projets qui lui appartiennent, rôle de l'utilisateur courant. Accessible en cliquant sur une carte depuis Organisations.
